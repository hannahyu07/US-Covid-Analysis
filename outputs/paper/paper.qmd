...
ghs_data <- read_excel("C:/Users/35911/Documents/US-Covid-Analysis/inputs/data/GHS_index.xlsx")
world_map <- st_read("C:/Users/35911/Documents/US-Covid-Analysis/inputs/data/world/World_Countries__Generalized_.shp")
life_exp <- read.csv("C:/Users/35911/Documents/US-Covid-Analysis/inputs/data/life_table.csv", stringsAsFactors = FALSE)
vote_share <- read.csv("C:/Users/35911/Documents/US-Covid-Analysis/inputs/data/Vote_share_2020_data.csv", stringsAsFactors = FALSE)
deaths <- read.csv("C:/Users/35911/Documents/US-Covid-Analysis/inputs/data/covid-19-death-rates.csv", stringsAsFactors = FALSE)
...
The datasets utilized in this paper were mainly obtained from the `original paper` [@citearticle]. The data used by the paper was compiled by various sources such as the @data1, @data2, @data3, @data4, @data5, @data6, @data7, @data8, @data9, @data10, and @data11. Additionally, to address the original paper's lack of US COVID statistics and political party support data, we incorporated information from @citedata1 and @citedata2.The political party support data we utilized from @citedata1 was compiled from @data12. 
...
This paper will to replicate the data that was originally presented by @citearticle, as previously mentioned. We then transform the replication to our own data analysis incorporating the COVID statistics adn political party support data. We seek to use our initial replication and following own data anlysis to provide a fresh part of the original paper, enhancing the with more data. Different from the original paper that spend a lot of time talking about how badly prepared US was against other country, We also specifically diverge our focus of the political and socioeconomic factors that impacted US's COVID outcome.
...
Since we aim to analyze people's political preference on a state level and the data we collected from @data12 is on a county level, we create a new dataframe "vote_data_by_state" to summarize all the votes by state. We utilize the "group_by" function to form new variables on a state level. We calculate each of the sum of the total votes, total Republican party votes, total Democratic party votes, total vote difference between the two parties by county and created new variables that calculate the sum of each of these. We also create variables on the average percentage of vote each party receives and the difference by each state.

To find the total number of votes in Top 10 states with highest proportion of Republican votes, we create another new dataframe "sorted_states" from the "vote_data_by_state" dataframe. We include a new variable "proportion_republican" that calculates the proportion of votes the Republican party receives from each state and ordered the states by the highest proportion of Republican votes. 

@datadata2 does not have column names for each column.To retrieve data by column for we assigned column names "State" and "Death_Rate" to the variables of interests. And then, to find the top 10 states with highest death rates, we ranked the states based on death rate. 
...
Our results are summarized in the following figures. @fig-ELE illustrates the trend of life expectancy at birth across different racial groups over time. @tbl-life-exp provides a more detailed breakdown of the life expectancy before and during COVID for...  

Unfortunately, due to the unavailability of data over time for Asian, American Indian, and Alaska Native communities, they have been excluded from the time series graph.

An intriguing observation is the consistently higher life expectancy among Hispanic individuals compared to other groups, even amidst the challenges posed by COVID-19. On the other hand, Black individuals l have consistently exhibited lower life expectancy, which further declined notably in 2021, reaching just over 70.8 years old. The life expectancy trends of white people and all other races and origins remain close together throughout the 14 years, with minimum variance. Specifically, the life expectancy for White individuals decreased from 78.8 to 76.4 years old from pre-pandemic levels in 2019 to 2021, while for Hispanic individuals, it dropped by 4.2 years, and for Black individuals, it declined by 4 years during the same period.
...
table_gt <- gt(table) %>%
  tab_header(
    title = "Top 10 States with Highest Death Rates from COVID-19 (per 100,000 people)"
  )
...
life_exp_gt <- gt(life_exp_table, rowname_col = "Race") %>%
  tab_header(
    title = "Life Expectancy by Race (2019-2021)"
  )
# Print the gt table
life_exp_gt
